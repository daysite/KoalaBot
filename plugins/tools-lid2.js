...
• .lid2 @user1 @user2
    ...
    if (!targets.length) {
      return conn.reply(m.chat, 'No se encontraron usuarios', m)
    }
    ...
    let response = `> *ⓘ USUARIOS ENCONTRADOS*: ${results.length}\n\n`
    ...
    await conn.reply(m.chat, response.trim(), fkontak || m, { mentions: mentionJids })

  } catch (error) {
    console.error('Error en lid2:', error)
    conn.reply(m.chat, 'Error al procesar usuarios', m)
  }
}
